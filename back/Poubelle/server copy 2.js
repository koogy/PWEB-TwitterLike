...
/*   var query = "SELECT message.message_id,message.user_id,users.name AS username,message_content,message_date,(SELECT count(message_id) FROM message_user_like WHERE message_id = message.message_id AND liked='t' GROUP BY message_id ) AS liked,(SELECT count(followed) AS followed FROM follow WHERE followed ='t' and user_id= $1 AND usertofollow_id = message.user_id GROUP BY followed) AS follow,retweet.message_id as rt_message_user_id,retweet.user_id as rt_user_id,u2.name as rt_username FROM (((message join users ON message.user_id = users.id) left outer join retweet on message.message_id = retweet.message_id) left join users as u2 on retweet.user_id = u2.id)"
  ...
  var getLikeCount = " (SELECT count(message_id) FROM message_user_like WHERE message_id = message_id AND liked='t' GROUP BY message_id) AS liked"
  var getFollow = " (SELECT count(followed) AS followed FROM follow WHERE followed ='t' and user_id= $1 AND usertofollow_id = message.user_id GROUP BY followed) AS follow "
  
  var query_1 = " SELECT message.message_id,message.user_id,users.name AS username,message_content,message_date,"
      + getLikeCount + "," + getFollow
      + ",retweet.message_id as rt_message_user_id,retweet.user_id as rt_user_id,u2.name as rt_username FROM (((message join users ON message.user_id = users.id)"
      + "left outer join retweet on message.message_id = retweet.message_id) left join users as u2 on retweet.user_id = u2.id) where message.message_id != -1 "
  

  /* UNION */
  var query_2 = 
  "SELECT message.message_id, message.user_id, users.name AS username, message_content, message_date,"
  + getLikeCount + "," + getFollow+ ", '-1' as rt_message_user_id, '-1' as rt_user_id, '' as rt_username FROM(message join users ON message.user_id = users.id)  where message.message_id != -1"
 /*  console.log(query_1 +" UNION " + query_2) */
...
query_1 += filtersQuery
query_2 += filtersQuery

  if(followFilter){
    console.log("FOLLLOW FILTER !!")
     query_1 += " AND ((select count(followed) AS followed FROM follow WHERE followed ='t' AND user_id= $1 AND usertofollow_id = message.user_id group by followed)) =1"
     query_2 += " AND ((select count(followed) AS followed FROM follow WHERE followed ='t' AND user_id= $1 AND usertofollow_id = message.user_id group by followed)) =1"
  }
